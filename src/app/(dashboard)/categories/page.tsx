...
// ── Draggable Row ────────────────────────────────────────────────────────────
const DraggableRow = React.memo(({ cat, idx, usageCount, isSelected, onSelect, onEdit, onDelete, onToggle, onDragStart, onDragEnter, onDragEnd }: {
    cat: Category; idx: number; usageCount: number; isSelected: boolean;
    ...
    return (
        <div
            ...
            onDragEnd={() => { setIsDragging(false); setIsOver(false); dragCounter.current = 0; onDragEnd(); }}
            className={cn(
                "grid items-center gap-4 px-5 h-[52px] transition-all duration-200 select-none relative shrink-0",
                "grid-cols-[40px_28px_1fr_80px_100px_120px_80px]",
                isDragging ? "opacity-30 bg-slate-50 rounded-xl" : "",
                isOver ? "bg-blue-50/80 ring-2 ring-inset ring-blue-500 shadow-md z-10 rounded-xl" : "hover:bg-slate-100/80",
        ...
        >
            ...
            <div className="flex justify-center cursor-grab active:cursor-grabbing text-slate-400 hover:text-slate-700 transition-colors">
                <GripVertical size={18} strokeWidth={2.5} />
            </div>
            <div className="min-w-0">
                <div className="flex items-center gap-2.5">
                    <p className="text-[15px] font-semibold text-slate-700 m-0 truncate">{cat.name}</p>
                </div>
...
export default function CategoriesPage() {
    ...
    // [แก้] usageCount ถูก declare และจะถูก set จริงเมื่อ fetchAll ดึงข้อมูล
    ...
    // [แก้] fetchAll ใช้ Promise.all แทน sequential for-await
    // เหตุผล: ดึงทุก type พร้อมกันแทนทีละอัน ลด loading time
    const fetchAll = useCallback(async () => {
        ...
        try {
            ...
            // [แก้] ดึง usageCount จาก API ถ้ามี endpoint
            // ถ้ายังไม่มี endpoint ให้ comment บรรทัดนี้ออกได้
            try {
                ...
                // ถ้า endpoint ยังไม่มี ไม่ทำอะไร usageCount จะเป็น 0 ทั้งหมด
            }
        } catch {
            ...
            <div className="min-h-screen bg-slate-100 -m-6">
                ...
                <main className="pt-8 pb-5 px-10 h-[calc(100vh-52px)] w-full">
                    <div className="flex flex-col bg-white rounded-xl border border-slate-200 overflow-hidden h-full" style={{ boxShadow: "0 0 40px rgba(0,0,0,0.06), 0 0 20px rgba(0,0,0,0.04), 0 2px 10px rgba(0,0,0,0.02)" }}>
                        {/* ── Card Header ── */}
                        <div className="px-6 py-5 border-b border-slate-200 shrink-0 bg-slate-50/30 flex items-center justify-between relative overflow-hidden h-[76px]">
                            <h1 className="text-[22px] font-extrabold text-[#0f172a] tracking-tight m-0 leading-tight shrink-0">จัดการหมวดหมู่</h1>

                            {/* Bulk Action Bar (Sliding Overlay - Right Side) */}
                            <div className="absolute top-0 right-0 bottom-0 left-[259.3px] pointer-events-none">
                                <AnimatePresence>
                                    {selectedIds.length > 0 && (
                                        <motion.div
                                            ...
                                            exit={{ y: -80, opacity: 0 }}
                                            transition={{ type: "spring", damping: 25, stiffness: 300 }}
                                            className="absolute inset-0 z-50 flex items-center justify-start gap-4 px-6 py-2 bg-white border-l border-slate-200 pointer-events-auto shadow-[inset_0_-2px_10px_rgba(37,99,235,0.03)]"
                                        >
                                            <div className="flex items-center gap-4 min-w-0">
                                                ...
                                                <div className="min-w-0">
                                                    <p className="text-[17px] font-bold text-slate-900 m-0 leading-tight truncate">จัดการกลุ่มรายการ</p>
                                                    <p className="text-[13px] text-blue-600 font-bold m-0 mt-0.5 animate-pulse">
                                            ...
                                            <div className="flex items-center gap-2 shrink-0">
                                                <button
                                                    ...
                                                    className="px-4 py-2 text-[13px] font-bold text-slate-500 hover:text-slate-700 hover:bg-slate-50 rounded-lg transition-all cursor-pointer border border-slate-200 bg-white"
                                                >
                                                    ยกเลิก
                                                </button>
                                                <button
                                                    onClick={() => setIsBulkDelete(true)}
                                                    className="flex items-center gap-2 px-5 py-2 bg-red-500 hover:bg-red-600 text-white text-[13px] font-bold rounded-lg transition-all cursor-pointer border-none shadow-lg shadow-red-500/20 active:scale-95"
                                                >
                                                    <Trash2 size={15} />
                                                    ลบที่เลือก
                                                </button>
                        ...
                        {/* ── Card Body (Sidebar + Content) ── */}
                        <div className="flex items-stretch flex-1 min-h-0">
                            {/* Sidebar */}
                            <aside className="w-[260px] shrink-0 border-r border-slate-200 flex flex-col overflow-hidden h-full bg-slate-50/30">
                                <div className="flex items-center gap-3 px-5 border-b-[1.5px] border-slate-200 shrink-0 h-[76px]">
                                ...
                                {/* Type list */}
                                <div className="flex flex-col gap-px px-2.5 py-2 overflow-y-auto flex-1 custom-scrollbar">
                                    {categoryTypes.map(type => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={cn(
                                                    // [ปรับ UI] เปลี่ยนจาก rounded-xl เป็น rounded-full (pill)
                                                    // ทำให้ดูสะอาดตาและชัดเจนขึ้น
                                                    "w-full px-3 h-[44px] shrink-0 rounded-lg text-left transition-all flex items-center justify-between group cursor-pointer",
                                                    isActive
                                                        ? "bg-blue-50 border-blue-600 text-blue-600 shadow-sm"
                                                        : "text-slate-600 hover:bg-slate-50 border-transparent hover:border-slate-300 hover:text-slate-900"
                                                )}
                                            >
                                                <div className="flex items-center gap-3 flex-1 min-w-0 px-1">
                                                    ...
                                                    <span className="text-[14px] font-semibold truncate">{type.label}</span>
                                                </div>

                                                <div className="flex items-center gap-1 ml-1 shrink-0">
                                                    {/* badge จำนวน */}
                                                    <span className={cn(
                                                        "text-xs font-bold px-1 py-0.5 min-w-[20px] text-center bg-transparent transition-colors",
                                                        isActive ? "text-blue-600" : "text-slate-400 group-hover:text-slate-700"
                                                    )}>
                                                        {counts[type.id] ?? 0}
                                                    </span>
                            ...
                            <div className="flex-1 min-w-0 flex flex-col overflow-hidden h-full bg-white relative">
                                {/* Toolbar */}
                                <div className="flex items-center justify-between px-6 border-b-[1.5px] border-slate-200 shrink-0 h-[76px]">
                                            ...
                                            return Icon ? (
                                                <>
                                                    ...
                                                    <div className="flex flex-col items-start gap-1">
                                                        <h2 className="text-[19px] font-bold text-slate-900 m-0 leading-none">{cur?.label}</h2>
                                                        <p className="text-[13px] text-slate-500 m-0 leading-none mt-1">{filtered.length} รายการ</p>
                                                    </div>
                                                    <button onClick={() => setShowGuidance(true)}
                                                        className="ml-2 group flex items-center gap-1.5 px-2.5 py-1.5 bg-blue-50/80 hover:bg-blue-100 text-blue-700 rounded-lg text-xs font-bold transition-all cursor-pointer border border-blue-200 hover:border-blue-300 shadow-sm active:scale-95">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <div className="relative">
                                            ...
                                            <input
                                                className="pl-8 pr-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-[13px] outline-none w-48 hover:border-blue-600 focus:border-blue-600 transition-all disabled:opacity-50 text-slate-900"
                                                placeholder="ค้นหา..." value={search}
                                                onChange={e => setSearch(e.target.value)}
                                                disabled={categoryTypes.length === 0} />
                                        </div>
                                        <button onClick={() => { setEditTarget(null); setShowAddItem(true); }}
                                            disabled={categoryTypes.length === 0}
                                            className={cn(
                                                "group flex items-center gap-1.5 px-4 py-2 rounded-lg text-[13px] font-bold transition-all duration-200 whitespace-nowrap border-none",
                                                categoryTypes.length === 0
                                                    ? "bg-slate-100 text-slate-400 cursor-not-allowed"
                                                    : "bg-blue-600 hover:bg-blue-500 text-white shadow-md active:scale-[0.98] cursor-pointer"
                                            )}>
                                            <Plus size={15} className={cn(categoryTypes.length > 0 && "group-hover:scale-110 transition-transform duration-200")} />
                                            เพิ่มรายการ
                                        </button>
                                    </div>
                                </div>
                                ...
                                {/* Table header */}
                                <div className="grid items-center gap-4 px-5 py-3.5 bg-[#fafafa] border-b-[1.5px] border-slate-200 text-[14px] font-bold text-slate-700 grid-cols-[40px_28px_1fr_80px_100px_120px_80px] h-[52px]">
                                    <div className="flex justify-center">
                                        <div onClick={toggleSelectAll}
                                            className={cn(
                                                "w-5 h-5 rounded-md border-2 transition-all flex items-center justify-center cursor-pointer",
                                                selectedIds.length > 0 && selectedIds.length === filtered.length ? "bg-blue-600 border-blue-600 shadow-sm" : "bg-white border-slate-300 hover:border-blue-400"
                                            )}>
                                    ...
                                    <div />
                                    <div>ชื่อ</div>
                                    <div className="text-center">ใช้งาน</div>
                                    <div className="text-center">สถานะ</div>
                                    <div>สร้างเมื่อ</div>
                                    <div className="text-end">จัดการ</div>
                                </div>
                                ...
                                <div className="divide-y divide-slate-200 overflow-y-auto custom-scrollbar flex-1">
                                    {loading ? (
                                        [...Array(5)].map((_, i) => (
                                            <div key={i} className="h-14 px-5 animate-pulse bg-slate-50/60" />
                                        ))
                                    ) : categoryTypes.length === 0 ? (
                                        <div className="py-20 text-center">
                                            ...
                                            <p className="text-sm font-bold text-slate-400 m-0">กรุณาติดต่อผู้ดูแลระบบเพื่อสร้าง "ประเภทหมวดหมู่"</p>
                                        </div>
                                    ) : filtered.length === 0 ? (
                                        <div className="py-20 text-center">
                                            <div className="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center mx-auto mb-3">
                                            ...
                                            <p className="text-sm font-bold text-slate-400 m-0">ยังไม่มีรายการในหมวดหมู่นี้</p>
                                            <p className="text-xs text-slate-400 mt-1">คลิก "เพิ่มรายการ" เพื่อเริ่ม</p>
                                        </div>
                                    ) : (
                                        <>
