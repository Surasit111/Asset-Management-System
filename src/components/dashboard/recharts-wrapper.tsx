...
export default function RechartsWrapper({ data, type, collapsed, getBarColor, isMoney = false }: any) {
    if (!data || data.length === 0) {
        return <div className="h-full flex items-center justify-center text-sm text-gray-300">ยังไม่มีข้อมูล</div>;
    }

    if (isMoney) {
        return (
            <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 80 }} accessibilityLayer={false} style={{ outline: "none", border: "none" }} tabIndex={-1}>
